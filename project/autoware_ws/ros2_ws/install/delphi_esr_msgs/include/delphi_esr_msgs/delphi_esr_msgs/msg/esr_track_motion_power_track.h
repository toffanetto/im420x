// generated from rosidl_generator_c/resource/idl.h.em
// with input from delphi_esr_msgs:msg/EsrTrackMotionPowerTrack.idl
// generated code does not contain a copyright notice

#ifndef DELPHI_ESR_MSGS__MSG__ESR_TRACK_MOTION_POWER_TRACK_H_
#define DELPHI_ESR_MSGS__MSG__ESR_TRACK_MOTION_POWER_TRACK_H_

#include "delphi_esr_msgs/msg/detail/esr_track_motion_power_track__struct.h"
#include "delphi_esr_msgs/msg/detail/esr_track_motion_power_track__functions.h"
#include "delphi_esr_msgs/msg/detail/esr_track_motion_power_track__type_support.h"

#endif  // DELPHI_ESR_MSGS__MSG__ESR_TRACK_MOTION_POWER_TRACK_H_
