  ...
  autoware_vehicle_msgs__msg__TurnIndicatorsReport__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_vehicle_msgs__msg__TurnIndicatorsReport * data =
      (autoware_vehicle_msgs__msg__TurnIndicatorsReport *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_vehicle_msgs__msg__TurnIndicatorsReport__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          autoware_vehicle_msgs__msg__TurnIndicatorsReport__fini(&output->data[i]);
        }
        return false;
      }
    }
